...
사용법:
  python3 run_arb.py                         # 모의거래 (기본, 실거래 안 됨)
  ...
  python3 run_arb.py --live                  # 실거래 모드 (주의!)
"""
...
def main():
    ...
    parser.add_argument("--live", action="store_true",
                        help="실거래 모드 활성화 (기본: 모의거래)")
    parser.add_argument("--log-level", default="INFO")

    args = parser.parse_args()

    if args.live:
        print("⚠️  재정거래봇을 실거래 모드로 실행합니다! 실제 자금이 사용됩니다.")
        ...
        print("📊 모의거래 모드로 실행합니다. 실거래는 --live 플래그를 추가하세요.")
