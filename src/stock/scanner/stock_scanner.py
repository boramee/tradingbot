...
class StockScanner:
    ...
    def _stage1_volume_scan(self) -> List[ScanResult]:
        ...
        for item in rankings:
            ...
            # ETF/ETN 제외 (인버스/레버리지 등 파생상품)
            name = item.get("name", "")
            if code.startswith("Q") or code.startswith("5"):
                # ETN 코드는 Q 또는 5로 시작
                logger.debug("[스캐너 1단계] 제외(ETN): %s %s", code, name)
                skip_low_value += 1
                continue
            ...
            if any(kw in name for kw in etf_keywords):
                logger.debug("[스캐너 1단계] 제외(ETF/ETN): %s %s", code, name)
                skip_low_value += 1
                continue
