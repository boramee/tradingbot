...
class StockEngine(BaseTradingEngine):
    ...
    def __init__(
        ...
        # 사용자 요청: 코스피 급락 필터 기본 비활성
        self.enable_kospi_crash_filter = False
    ...
    def _check_market_conditions(self) -> tuple:
        if not self.enable_kospi_crash_filter:
            return True, ""
        now = time.time()
    ...
    def start(self, poll_sec: int = 10):
        ...
        if self.enable_kospi_crash_filter:
            logger.info("  필터: 코스피급락 + 수급 + 체결강도 + VI + 갭")
        else:
            logger.info("  필터: 수급 + 체결강도 + VI + 갭 (코스피급락 필터 OFF)")
